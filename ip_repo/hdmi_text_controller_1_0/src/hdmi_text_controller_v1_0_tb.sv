...
module hdmi_text_controller_tb();
    ...
    task axi_read (input logic [31:0] addr, output logic [31:0] data);
        begin
            ...
            read_data_ready <= 1'b1;    //indicates data is ready

            //wait for slave to assert address is ready to read
            wait (read_addr_ready);
            
             @(posedge aclk);
            //Case: Handshake completed (read_addr_valid && read_addr_ready == 1)
            read_addr_valid <= 1'b0;
            
            wait (read_data_valid);
            
            @(posedge aclk);
            //Case: Transaction compelted (read_data_ready && read_data_valid == 1)
            data <= read_data;
