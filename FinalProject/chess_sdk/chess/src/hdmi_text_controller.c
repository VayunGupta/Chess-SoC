...
{
	uint32_t pos = color >> 1;			// Every even and odd color are mapped to the same pos

	const uint32_t ODD_MASK  = ~(0x1FFE000); // Clear bits 24-13
    const uint32_t EVEN_MASK = ~(0x00001FFE); // Clear bits 12-1

	if (color % 2) {

	    uint32_t  oddColorBits = ((uint32_t) red << 21) | ((uint32_t) green << 17) | ((uint32_t) blue << 13);

		// Clear the bits for the odd color, store the calculated bits in that position
	    hdmi_ctrl->color_palette[pos] &= ODD_MASK;
	    hdmi_ctrl->color_palette[pos] |= oddColorBits;
	}
	else {
	    uint32_t  evenColorBits = ((uint32_t) red << 9) | ((uint32_t) green << 5) | ((uint32_t) blue << 1);

		// Clear the bits for the even color, store the calculated bits in that position
	    hdmi_ctrl->color_palette[pos] &= EVEN_MASK;
	    hdmi_ctrl->color_palette[pos] |= evenColorBits;
	}
}
