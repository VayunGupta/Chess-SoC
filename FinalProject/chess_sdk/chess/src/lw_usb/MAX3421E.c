...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	//select MAX3421E
			...
			// * @param	InstancePtr is a pointer to the XSpi instance to be worked on.
					// We are setting this to &SpiInstance, which has been initialised elsewhere in the code
			...
			//	*		SlaveMask can be zero if the slave is being deselected.
					// We are setting this to 1, since we only have 1 Spi source - the keyboard

	int err =  XSpi_SetSlaveSelect(&SpiInstance, 1);
	if (err != XST_SUCCESS)
	{
		xil_printf("Failed with error code %d \n", err);
		return 0;
	}
	...
	//write val via SPI
	u8 sendData[2];
	sendData[0] = reg | 0x02;				// This works because a BYTE is the same as a u8
	sendData[1] = val;

	err = XSpi_Transfer(&SpiInstance, sendData, NULL, 2); // 2 for 2 bytes being sent
	...
	err =  XSpi_SetSlaveSelect(&SpiInstance, 0);
	if (err != XST_SUCCESS)
		{
			xil_printf("Failed with error code %d \n", err);
			return 0;
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	err = XSpi_Transfer(&SpiInstance, sendData, NULL, nbytes+1); // 2 for 2 bytes being sent
...
//			*		number of bytes received always equals the number of bytes sent.
	...
	u8 sendData[2];
	sendData[0] = reg;				// This works because a BYTE is the same as a u8
	sendData[1] = 0;
	u8 recieveData [2];
	recieveData[0] = 0;

	err = XSpi_Transfer(&SpiInstance, sendData, recieveData, 2);

	//read val via SPI
	BYTE val = recieveData[1];
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	u8 recieveData [nbytes+1];

	err = XSpi_Transfer(&SpiInstance, sendData, recieveData, nbytes+1);

	//read data[n] from SPI, where n goes from 0 to nbytes-1
	for (int i = 0; i<nbytes; i++)
	{
		data[i] = recieveData[i+1];
	}
