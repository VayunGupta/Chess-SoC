...
struct TEXT_HDMI_STRUCT {
		...
		// We need color palette to begin at x2000. The VRAM is Rows * Cols * 2, so it takes up space till x12C0.
		// As in the table 8 of the Intro to AXI Doc, we need to have a 'unused but reserved by Vivado' buffer space
		//  so that the color palette can begin at x2000 and not x12C0.

		// x2000 - x12C0 = x0D40.
		// This is 3392 in decimal
		const uint16_t reserved_by_vivado[3392];
